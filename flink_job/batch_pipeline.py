...
print ("SINK_KAFKA_TOPIC: ", SINK_KAFKA_TOPIC)
consumer = FlinkKafkaConsumer(SOURCE_KAFKA_TOPIC, SimpleStringSchema(), consumer_props)
#consumer = FlinkKafkaConsumer(SOURCE_KAFKA_TOPIC, JsonRowDeserializationSchema(), consumer_props)

## debugging Raman
#input_stream = env.add_source(consumer)
#input_stream.map(lambda value: print(value)).name("print")
